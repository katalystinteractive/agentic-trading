"""Tests for the reactive dependency graph — engine, builder, and integration.

Covers all 93 test cases from plans/graph-test-analysis.md:
A1-A23: Helper functions
B1-B13: Graph engine mechanics
C1-C19: Graph builder (nodes, edges, values)
D1-D8:  Signal propagation chains
E1-E10: Persistence schema
F1-F11: Dashboard bucket categorization
G1-G6:  Error handling
H1-H3:  Design constraints
...
from graph_builder import (
    ...
    _format_action_reason, _stub,
)


# ---------------------------------------------------------------------------
# Fixtures (from test analysis Section 2)
# ---------------------------------------------------------------------------
...
FIXTURE_PORTFOLIO = {
    "positions": {
        "CLSK": {"shares": 42, "avg_cost": 8.99, "entry_date": "2026-03-23",
                 "note": "", "bullets_used": 5,
                 "fill_prices": [9.89, 9.74, 9.05, 8.91, 8.40],
                 "target_exit": None},
...
class TestSignalPropagation:
    ...
    def test_d5_catastrophic_resolved(self):
        graph = _build_fixture_graph()
        # Set prev on BOTH decision node and report node
        graph.load_prev_state({"CLSK:catastrophic": "WARNING",
                               "CLSK:catastrophic_alert": "WARNING"})
        graph.propagate_signals()
        cat = graph.nodes.get("CLSK:catastrophic_alert")
        # Was WARNING, now None — should have changed
...
class TestPersistenceSchema:
    ...
    def test_e10_verdict_matches_graph(self):
        ...
        state = get_state_for_persistence(
            graph, ["CLSK"],
            FIXTURE_PORTFOLIO.get("pending_orders", {}))
        # Persistence stores tuples as-is; compare as tuples
        assert tuple(state["tickers"]["CLSK"]["verdict"]) == tuple(graph.nodes["CLSK:verdict"].value)


# ---------------------------------------------------------------------------
# F: Dashboard Buckets (F1-F11)
# ---------------------------------------------------------------------------
