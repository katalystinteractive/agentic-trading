...
**Pre-check:** If a position has `note` containing "recovery" or "pre-strategy" BUT current P/L > 0%, verify the analyst treated it as **non-recovery** (the recovery is complete). Flag if recovery rules (3-5, 8-10) were applied to a position with P/L > 0%.
...
7. **Positions with 10% <= P/L < 12% + earnings NOT GATED** must NOT get EXIT or REDUCE verdict (they are at target — should be HOLD).
7a. **Positions with P/L >= 12% + earnings NOT GATED** must be REDUCE (at or past top of target range — take profits). Flag HOLD or MONITOR as incorrect.
...
10. **HOLD for time-stop-exceeded non-recovery positions** requires explicit bullish justification AND earnings CLEAR in the Reasoning field. Flag if missing or generic. Flag HOLD if earnings is APPROACHING (7-14 days) for non-recovery positions with P/L < 7% — should be REDUCE per analyst rule 13. Exception: if P/L >= 7% (profit target AT TARGET or APPROACHING per rules 7-8), profit target rules take precedence — HOLD is correct and should not be flagged. (Recovery positions with time stop EXCEEDED are handled by rules 8-10 where HOLD is valid regardless of earnings status.)
11. **Recovery positions** must NOT get EXIT verdict regardless of earnings status. For non-GATED recovery: valid verdicts are HOLD or MONITOR only — flag EXIT or REDUCE. (Recovery positions are evaluated by analyst rules 8-10 before time-stop rules 11-14 can fire — the "first match wins" ordering ensures recovery never reaches the time-stop REDUCE path.) For GATED recovery: apply rules 3-5 above.
