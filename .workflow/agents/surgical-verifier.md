...
Then write `candidate-verification.json` with the following schema:
  ...
  "candidates": [
    {
      ...
      "adjusted_score": 100,
      "verdict": "PASS",
      "key_finding": "All arithmetic and data verified; 9% gap, 12 approaches at B1 confirmed"
    }
  ]
}
```
...
- `ticker` — string
- `original_score` — int (from shortlist)
- `adjustment` — int (your +/-10 qualitative adjustment)
- `adjusted_score` — int (original + adjustment)
- `verdict` — string, one of `"PASS"`, `"FLAG"`, `"FAIL"`
- `key_finding` — string (one-line summary matching the Verification Summary Table)

The JSON must contain one entry per candidate in the same order as the markdown table. Use the current ISO-8601 timestamp for `generated`.

**IMPORTANT:** Output the HANDOFF decision marker IMMEDIATELY after writing both files. Do NOT re-read or verify the files.
...
- `candidate-verification.json` — structured verification data (scores, adjustments, verdicts) for downstream tools
...
**Artifacts:** candidate-pre-verify.md, candidate-verification.md, candidate-verification.json
