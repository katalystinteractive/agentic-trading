...
def get_top_catalyst(catalysts):
    """Return top catalyst label with secondary when primary is Earnings.

    Sorts by count descending, then category alphabetically for ties.
    When the top category is "Earnings" and a second category exists,
    returns "Earnings / Secondary" format.
    """
    if not catalysts:
        return "\u2014"
    sorted_cats = sorted(catalysts, key=lambda c: (-c["count"], c["category"]))
    top = sorted_cats[0]["category"]
    if top == "Earnings" and len(sorted_cats) > 1:
        return f"Earnings / {sorted_cats[1]['category']}"
    return top
...
def build_heatmap(raw_data):
    ...
    for ticker, data in raw_data["tickers"].items():
        ...
        top_catalyst = get_top_catalyst(data["catalysts"])
...
def build_report(raw_data, heatmap_rows, distribution, flags, themes, recs):
    ...
    lines.append("*Generated by news_sweep_pre_analyst.py | Mechanical pre-processing for Phase 2*")
    lines.append("")
